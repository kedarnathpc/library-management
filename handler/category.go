...
func (h *Handler) listCategories(rw http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < totalPage; i++ {
		paginate[i] = CategoryPagination{
			URL:        fmt.Sprintf("http://localhost:%s/category/list?page=%d", port, i+1),
			PageNumber: i + 1,
		}
		if i+1 == p {
			if i != 0 {
				previousPageURL = fmt.Sprintf("http://localhost:%s/category/list?page=%d", port, i)
			}
			if i+1 != totalPage {
				nextPageURL = fmt.Sprintf("http://localhost:%s/book/list?page=%d", port, i+2)
			}
		}
	}
	list := ListCategory{
