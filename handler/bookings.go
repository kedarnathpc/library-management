...
func (h *Handler) myBookings(rw http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < totalPage; i++ {
		bookingPaginate[i] = BookingPagination{
			URL:        fmt.Sprintf("http://localhost:%s/mybookings?page=%d", port, i+1),
			PageNumber: i + 1,
		}
		if i+1 == p {
			if i != 0 {
				previousPageURL = fmt.Sprintf("http://localhost:%s/mybookings?page=%d", port, i)
			}
			if i+1 != totalPage {
				nextPageURL = fmt.Sprintf("http://localhost:%s/mybookings?page=%d", port, i+2)
			}
		}
	}
